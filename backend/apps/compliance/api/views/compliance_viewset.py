...
from apps.compliance.api.serializers.compliance_serializer import ComplianceSnapshotSerializer


class ComplianceViewSet(viewsets.ReadOnlyModelViewSet):

    queryset = ComplianceSnapshot.objects.select_related(
        "aircraft").order_by("-last_evaluated")
